...
class ViewController: UIViewController,UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    ...
    @objc func handleSignUp(){
        ...
        Auth.auth().createUser(withEmail: email, password: password) { (user, error: Error?) in
            ...
            print("Successfully created new user", user?.user.uid ?? "")
            ...
            storageRefChild.putData(uploadData, metadata: nil, completion: { (metadata, err) in
                ...
                storageRefChild.downloadURL(completion: { (url, err) in
                    if let err = err {
                        print ("Failed to get url: ", err.localizedDescription)
                        return
                    }
                    
                    guard let profileImageUrl = url?.absoluteString else { return }
                    print("Successfully get imageUrl: ", profileImageUrl)
    ...
    fileprivate func setupInputFields(){
        ...
        // alternative than using .isActive = true
...
extension UIView {
    func anchor(top: NSLayoutYAxisAnchor?, left: NSLayoutXAxisAnchor?, bottom: NSLayoutYAxisAnchor?, right: NSLayoutXAxisAnchor?, paddingTop: CGFloat, paddingLeft: CGFloat,  paddingBottom: CGFloat, paddingRight: CGFloat, width: CGFloat, height: CGFloat) {
        ...
        // mandatory, isActive is also mandatory
